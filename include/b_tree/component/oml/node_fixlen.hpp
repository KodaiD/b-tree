...
class NodeFixLen
{
 public:
   ...
   public:
    ...
    LockS()
    {
      ...
      while (true) {
        ...
        desired.slock_++;
        if (lock_word_.compare_exchange_weak(expected.obj_, desired.obj_,
    ...
    bool
    HasSameVersion(uint64_t ver) const
    {
      LockWord desired;
  ...
  auto
  CheckLeafNodeStatus(const size_t new_rec_len)  //
      -> NodeRC
  {
    NodeRC rc{};
  ...
  TryLockXForLeafNode()
  {
    return mutex_.TryLockX();
  }

  bool
  TryLockXForLeafNode(uint64_t ver)
  {
